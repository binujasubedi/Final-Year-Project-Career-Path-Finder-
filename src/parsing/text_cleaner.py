...
def clean_skill_list(skills: List[str]) -> List[str]:
    ...
    blocked_exact = {
        'skills', 'skill', 'career history', 'professional', 'teaching academic',
        'education', 'experience', 'work experience', 'employment history',
        'certifications', 'summary', 'profile', 'objective',
        'teaching', 'academic', 'board member', 'governance and implementation'
    }
    blocked_contains = [
        'accomplishment', 'responsible for', 'having ', 'years of',
        'background', 'employees provident fund', 'till now'
    ]
    ...
    for skill in skills:
        ...
        # Drop obvious section labels and sentence-like fragments.
        skill_lower = skill_clean.lower()
        if skill_lower in blocked_exact:
            continue
        if any(fragment in skill_lower for fragment in blocked_contains):
            continue
        if len(skill_clean.split()) > 4:
            continue
        if re.search(r'\d', skill_clean):
            continue
